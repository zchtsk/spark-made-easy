...
A partition does not working like an index in SQL. If you're working with customer transaction data, you might be tempted to partition on `customer_id`, but you'd end up with a partition for every customer. However, partitioning on `year-month` or `date` might be a good idea.
</Note>

This is just a starting point. For more information [check out the official docs](https://spark.apache.org/docs/latest/sql-data-sources-load-save-functions.html).